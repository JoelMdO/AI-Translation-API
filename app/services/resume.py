...
"""
# import re
##//TODO change by adding app before deploying 
...
class ResumeService:
    ...
    async def summarize(self, request: ResumeRequest) -> ResumeResponse:
        ...
        async def summarize_field(text: str) -> str:
            summary = await ollama_service.resume_article(
                request=text, model=OLLAMA_DEFAULT_MODEL or "llama3.2"
            )
        ...
        try:
            ...
            #     resume_article = summarize_field(request.article)
            #     resume_esArticle = summarize_field(request.esArticle)

            # else:
                # If no HTML, sanitize and process normally
            resume_article = await summarize_field(request.article)
            resume_es_article = await summarize_field(request.esArticle)
